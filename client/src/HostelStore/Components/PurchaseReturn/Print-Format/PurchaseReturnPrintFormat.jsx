...
import Logo from "../../../../../src/assets/NationalPrintLogo.jpeg";
import {
  findFromList,
...
const PurchaseReturnPrintFormat = ({
  ...
  const supplierDetails =
    singleData?.Supplier ||
    supplierList?.data?.find(
      (s) => parseInt(s.id) === parseInt(singleData?.supplierId),
    ) ||
    {};
  ...
  // ── Grand totals ──
  const totalPoQty = returnItems.reduce(
    (s, r) => s + (parseFloat(r.poQty) || 0),
    0,
  );
  const totalInwardQty = returnItems.reduce(
    (s, r) => s + (parseFloat(r.inwardQty) || 0),
    0,
  );
  const totalAlreadyReturnQty = returnItems.reduce(
    (s, r) => s + (parseFloat(r.alreadyReturnQty) || 0),
    0,
  );
  const totalBalQty = returnItems.reduce(
    (s, r) => s + (parseFloat(r.balQty) || 0),
    0,
  );
  ...
  return (
    <Document>
      {pageChunks.map((chunkRows, pageIndex) => {
        ...
        // page sub-totals
        const pgReturnQty = chunkRows.reduce(
          (s, r) => s + (parseFloat(r.returnQty) || 0),
          0,
        );
                ...
                <>
                  ...
                  <View style={styles.metaRow}>
                    {[
                      ...
                      {
                        label: "Return Date",
                        value: getDateFromDateTimeToDisplay(docDate),
                      },
                      { label: "DC No", value: dcNo },
                      {
                        label: "DC Date",
                        value: dcDate
                          ? getDateFromDateTimeToDisplay(dcDate)
                          : "",
                      },
                    ].map(({ label, value }) => (
                    ...
                    <View style={styles.colHalf}>
                      ...
                      <View style={styles.sectionBody}>
                        ...
                        {[
                          {
                            label: "Mobile No",
                            value: supplierDetails?.contactPersonNumber,
                          },
                ...
                {chunkRows.map((row, index) => {
                  ...
                  return (
                    <View key={globalOffset + index} style={rowStyle}>
                      ...
                      <Text
                        style={[styles.td, { flex: 3.5, textAlign: "left" }]}
                      >
                        {row?.StyleItem?.name ||
                          findFromList(
                            row.styleItemId,
                      ...
                      >
                        {row?.inwardQty
                          ? parseFloat(row.inwardQty).toFixed(2)
                          : ""}
                      </Text>
                      <Text
                      ...
                      >
                        {row?.returnQty
                          ? parseFloat(row.returnQty).toFixed(2)
                          : ""}
                      </Text>
                    </View>
                  );
                ...
                {Array.from({ length: emptyCount }).map((_, i) => {
                  ...
                  return (
                    <View key={`empty-${i}`} style={rowStyle}>
                      <Text
                        style={[styles.td, { flex: 0.5, color: "transparent" }]}
                      >
                        {" "}
                      </Text>
                      ...
                      <Text style={[styles.td, { flex: 1 }]}> </Text>
                      <Text
                        style={[styles.td, { flex: 1, borderRight: "none" }]}
                      >
                        {" "}
                      </Text>
                    </View>
